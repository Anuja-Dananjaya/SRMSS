...
const getAllDrivers = (req, res) => {
  const scope = depotWhere(req, 'd');
  const query = `
    SELECT d.*, dep.name AS depotName, u.email, u.name as userName 
    FROM drivers d
...
const getDriverById = (req, res) => {
  ...
  db.query(query, [id], (err, results) => {
    ...
    if (!sameDepotOrSuperAdmin(req, results[0].depotId)) {
      return res.status(403).json({ message: 'Driver belongs to another depot.' });
    }
...
const createDriver = (req, res) => {
  ...
  db.query('SELECT * FROM drivers WHERE licenseNo = ?', [licenseNo], (err, results) => {
    ...
    const query = `
      INSERT INTO drivers (depotId, name, licenseNo, licenseExpiry, phone, address, maxHoursPerDay, userId)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `;
