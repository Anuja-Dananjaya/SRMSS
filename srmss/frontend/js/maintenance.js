...
document.addEventListener('DOMContentLoaded', () => {
    initPage().then(() => {
        renderDepotFilter((depotId) => initMaintenancePage(depotId));
        initMaintenancePage(getSelectedDepotId());
    });
});



async function initMaintenancePage(depotId = '') {
    await Promise.all([
        loadVehicles(depotId),
        loadMaintenance(depotId)
    ]);
}
...
async function loadMaintenance(depotId = '') {
...
async function saveMaintLog() {
    ...
    try {
        ...
        if (data.success) {
            closeMaintModal();
            loadMaintenance(getSelectedDepotId());
        } else {
