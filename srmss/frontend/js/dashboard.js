...
document.addEventListener('DOMContentLoaded', () => {
    setCurrentDate();
    initPage().then(() => {
        renderDepotFilter((depotId) => loadDashboard(depotId));
        loadDashboard(getSelectedDepotId());
    });
});
...
async function loadDashboard(depotId = '') {
    document.getElementById('loadingState').style.display = 'block';
    document.getElementById('dashboardContent').style.display = 'none';

    const url = depotId ? `${API}/dashboard?depotId=${depotId}` : `${API}/dashboard`;
        ...
        if (result.success) {
            ...
            document.getElementById('loadingState').innerHTML =
                `<i class="fas fa-exclamation-circle" style="font-size:32px; color:#ef4444;"></i>
                 <p style="color:#ef4444; margin-top:16px;">${result.message || 'Failed to load dashboard.'}</p>`;
        }
    } catch (err) {
        document.getElementById('loadingState').innerHTML =
