...
document.addEventListener('DOMContentLoaded', () => {
    initPage().then(() => {
        renderDepotFilter((depotId) => loadVehicles(depotId));
        loadVehicles(getSelectedDepotId());
    });
});

// Listen for storage events to refresh vehicles when other pages trigger changes
window.addEventListener('storage', (e) => {
    if (e.key === 'vehiclesRefresh') {
        loadVehicles(getSelectedDepotId());
    }
});

// Also poll localStorage for updates (covers cases where storage events may not fire)
let __lastVehiclesRefresh = null;
function startVehiclesRefreshWatcher() {
    try { __lastVehiclesRefresh = localStorage.getItem('vehiclesRefresh'); } catch(e) { __lastVehiclesRefresh = null; }
    setInterval(() => {
        try {
            const v = localStorage.getItem('vehiclesRefresh');
            if (v && v !== __lastVehiclesRefresh) {
                __lastVehiclesRefresh = v;
                loadVehicles(getSelectedDepotId());
            }
        } catch (e) {
            // ignore
...
document.addEventListener('DOMContentLoaded', startVehiclesRefreshWatcher);
...
function formatStatus(status) {
    const map = { active: 'Active', maintenance: 'Maintenance', inactive: 'Inactive', on_trip: 'On Trip' };
