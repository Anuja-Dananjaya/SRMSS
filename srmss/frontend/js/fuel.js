...
document.addEventListener('DOMContentLoaded', () => {
    initPage().then(() => {
        renderDepotFilter((depotId) => initFuelPage(depotId));
        initFuelPage(getSelectedDepotId());
    });
});


async function initFuelPage(depotId = '') {
    // Load schedules first so we can determine which vehicles are scheduled
    await loadSchedules(depotId);
    await Promise.all([
        loadVehicles(depotId),
        loadFuelLogs(depotId)
    ]);
}
...
async function loadVehicles(depotId = '') {
    ...
    try {
        ...
        if (data.success) {
            ...
            // Determine vehicle IDs that appear in schedules
            const scheduledIds = new Set((allSchedules || []).map(s => s.vehicleId));
            const scheduledVehicles = allVehicles.filter(v => scheduledIds.has(v.vehicleId));
            select.innerHTML = '<option value="">-- Select Vehicle --</option>' +
                scheduledVehicles.map(v => `<option value="${v.vehicleId}">${v.registrationNo} - ${v.model}</option>`).join('');
        }
    } catch (err) {
        console.error(err);
    }
...
async function loadFuelLogs(depotId = '') {
    const url = depotId ? `${API}/fuel?depotId=${depotId}` : `${API}/fuel`;
    try {
        const res = await fetch(url, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
...
async function saveFuelLog() {
    ...
    try {
        ...
        if (data.success) {
            closeFuelModal();
            loadFuelLogs(getSelectedDepotId());
        } else {
