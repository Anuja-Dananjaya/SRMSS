...
document.addEventListener('DOMContentLoaded', () => {
    initPage().then(() => {
        renderDepotFilter((depotId) => loadAnalytics(depotId));
        loadAnalytics(getSelectedDepotId());
    });
});



async function loadAnalytics(depotId = '') {
    await Promise.all([
        loadFuelStats(depotId),
        loadMaintenanceStats(depotId),
        loadScheduleStats(depotId),
        loadActivityLogs(depotId)
    ]);
}

async function loadFuelStats(depotId = '') {
...
async function loadMaintenanceStats(depotId = '') {
    const url = depotId ? `${API}/maintenance?depotId=${depotId}` : `${API}/maintenance`;
    try {
        const res = await fetch(url, { headers: { 'Authorization': `Bearer ${token}` } });
...
async function loadScheduleStats(depotId = '') {
...
async function loadActivityLogs(depotId = '') {
    const url = depotId ? `${API}/activities?depotId=${depotId}` : `${API}/activities`;
    try {
        const res = await fetch(url, { headers: { 'Authorization': `Bearer ${token}` } });
